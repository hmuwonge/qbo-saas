...
//    }
    ...
    // Set route path based on type
    $paginator->setPath(($type === 'invoice') ? route('qbo.invoices.all') : route('qbo.receipts.index'));
    $itemsQbo = $paginator->items();
    $items_decode = json_decode(json_encode($itemsQbo), true);

    $filteredList = [];
    $filteredList[] = ($type === 'invoice')
      ? EfrisInvoiceSearch::findQbInvoicesByStatus($items_decode, 2, 2)
      : EfrisInvoiceSearch::findQbReceiptsByStatus($items_decode, 2, 2);
